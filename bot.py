...
return amount_token, '0x' + tx_hash.hex()[2:], decimalsSell token on Uniswapasync def sell_token(user_id, contract, amount_token, context=None, is_profit=False, chat_id=None, slippage=50):
    if user_id not in users:
        return "No wallet", 0
    address = users[user_id]['address']
    pk = users[user_id]['private_key']
...
return '0x' + tx_hash.hex()[2:], sold_usdMonitor tradesasync def monitor_trades(context: ContextTypes.DEFAULT_TYPE):
...
return ConversationHandler.ENDHandle non-command messagesasync def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.message.from_user.id)
